...
trong fishing_cog.py. Dùng tốn `energy_cost` thể lực ngay khi dùng, có 2
loại hiệu ứng chính:
...
class Skill:
    ...
    cooldown_s: float = 0.0           # thời gian hồi chiêu (giây) TRƯỚC KHI dùng lại được
        # skill này trong CÙNG 1 ván câu. KHÔNG còn giới hạn số lần dùng/ván
        # nữa — skill nào cũng dùng lại VÔ HẠN LẦN, chỉ cần hồi xong
        # cooldown (và đủ năng lượng) — xem ReelView._on_skill/
        # skill_cooldown_until trong fishing_cog.py.
...
SKILL_SHOP: list[Skill] = [
    ...
    Skill(
        ...
        cooldown_s=26, duration_s=6, bonus_damage_pct=0.25,
        description=(
